...
export async function voteReview(userId: string, reviewId: string, dir: "up" | "down") {
  ...
  if (existing) {
    ...
    await Vote.create({ reviewId, userId, value: target });
  }
  
  const allVotes = await Vote.find({ reviewId }).lean();
  const score = allVotes.reduce((sum, v) => sum + (v as any).value, 0);
  const myVote = allVotes.find(v => String((v as any).userId) === userId)?.value ?? 0;
